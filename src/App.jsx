...
function App() {
  ...
  useEffect(() => {
    ...
    loadAll();
    const channel = supabase.channel("nrs-demo-v6-realtime")
  ...
  async function loadAll() {
    ...
    const [p,t,s,a,r,au,l,sc,ai] = res;
    if (p.error) setBootError(p.error.message);
    setPatients(p.data || []); setTasks(t.data || []); setStaff(s.data || []); setAnnouncements(a.data || []);
    setReads(r.data || []); setAudit(au.data || []); setLeave(l.data || []); setSchedule(sc.data || []); setAiLogs(ai.data || []);
  }
    ...
    <main className="app">
      ...
      <header className="topbar">
        ...
        <div className="userbox">
          <strong>{profile.full_name}</strong><small>{roles[profile.role]}｜{profile.email}</small>
      ...
      <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
        ...
        <MenuGroup title="行政營運 / Admin" items={[["staff","人員與權限"],["leave","請假與代理"],["schedule","月排班與工時"],["announcements","公告與簽收"],["audit","數位足跡稽核"],["settings","Demo 管理設定"]]} view={view} setView={setAuthorizedView} can={can}/>
      </aside>
      ...
      <section className="content">
        ...
        {view === "schedule" && <Schedule schedule={schedule} seedMonthSchedule={seedMonthSchedule} selectedMonth={selectedMonth} setSelectedMonth={setSelectedMonth} />}
...
function Dashboard({ patients, tasks, announcements, reads, profile, generateFake }) {
  ...
  return <><PageTitle title="工作指揮中心" desc="雲端多人互動版本；資料存在 Supabase，OpenAI 行政助理可手動產生建議。" actions={<><button className="secondary" onClick={() => generateFake(20,false)}>追加 20 筆</button><button className="primary" onClick={() => generateFake(50,true)}>重置 50 筆</button></>} />
    ...
    <div className="split"><Card title="今日必辦">{tasks.slice(0,8).map(t => <TaskLine key={t.id} task={t}/>)}{!tasks.length && <p className="muted">目前無待辦。可先產生假資料。</p>}</Card><Card title="V6 更新重點"><p>已加入 OpenAI Edge Function、月排班、三個月預先請假卡位、AI 排班與任務摘要。</p></Card></div></>;
}
...
function Schedule({ schedule, seedMonthSchedule, selectedMonth, setSelectedMonth }) {
  const totals = {}; schedule.forEach(s => totals[s.staff_name] = (totals[s.staff_name] || 0) + Number(s.hours || 0));
  return <><PageTitle title="月排班與工時" desc="實務以月為單位排班；可先建立本月、下月與未來月份資料。" actions={<><select value={selectedMonth} onChange={e=>setSelectedMonth(e.target.value)}>{nextMonthOptions().map(m=><option key={m}>{m}</option>)}</select><button className="primary" onClick={seedMonthSchedule}>產生整月排班</button></>} />
